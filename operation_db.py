...
class Database:
    ...
    # 登录操作
    def log(self, name, passwd):
        sql = "select * from user where name='%s'" % name
        self.cur.execute(sql)
        r1 = self.cur.fetchone()  # 如果有查询结果则name存在
        ...
        passwd = "select * from user where passwd='%s'" % passwd
        self.cur.execute(passwd)
        r2 = self.cur.fetchone()  # 如果有查询结果则passwd存在
        if (r1 and r2):
            return True
        else:
            return False
